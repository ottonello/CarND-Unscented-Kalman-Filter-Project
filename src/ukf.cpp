...
void UKF::PredictStateAndCovariance() {//set weights_
  for (int i = 0; i < 2 * n_aug_ + 1; i++) {
    if (i == 0) {
...
void UKF::CreateAugmentedSigmaPoints(VectorXd &x_aug, MatrixXd &P_aug,
                                     MatrixXd &Xsig_aug) const {//create augmented mean state
  x_aug.head(5) = x_;
  x_aug(5) = 0;
  x_aug(6) = 0;
